...
const restaurantSelection = async (restaurantId: number, restaurantName: string): Promise<void> => {
  ...
  try {
    ...
    alert(`${restaurantName} added to your favorites!`);
  } catch (error) {
...
const initializePopup = (restaurant: Restaurant, marker: mapboxgl.Marker) => {
  ...
  favoriteButton.addEventListener("click", () => { restaurantSelection(Number(restaurant._id), restaurant.name) });
