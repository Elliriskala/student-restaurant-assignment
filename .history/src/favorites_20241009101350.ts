...
const addFavorite = async (
  ...
  try {
    ...
    const response = await fetchData<{ message: string; favouriteRestaurant: string[] }>(
        apiUrl + "/users/token", 
        options
    );

    const currentFavorites = response.favouriteRestaurant || [];
    ...
    const updateOptions: RequestInit = {
        ...
        body: JSON.stringify({ favouriteRestaurant: updatedFavorites }),
    };
