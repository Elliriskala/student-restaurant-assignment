...
const fetchFavorites = async (token: string): Promise<Restaurant[]> => {
  if (!token) { 
    newError('No token found');
    return [];
  }
  ...
  return await fetchData<Restaurant[]>(apiUrl + '/users/token', options);
};
...
export { checkLogin, fetchFavorites };