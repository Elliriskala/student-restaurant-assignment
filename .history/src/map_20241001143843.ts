...
const initializeMap = async () => {
  ...
  try {
    ...
    if (closestRestaurant) {
        ...
        const closestMarkerElement = document.createElement('div') as HTMLDivElement;
        closestMarkerElement.className = 'custom-marker closest';

        const closestMarker = new mapboxgl.Marker({element: closestMarkerElement})
        .setLngLat([longitude, latitude])
        .addTo(map);
        
        filteredMarkers.push(closestMarker);
