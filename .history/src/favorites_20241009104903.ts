...
const showFavorites = async (token: string): Promise<void> => {
  ...
  try {
    ...
    const response = await fetchData<{ data: { favoriteRestaurant: string[] } }>(
      apiUrl + `/users/token`,
      options
    );
    console.log("Users favorites fetched: ", response);

    const favoriteRestaurants = response.data.favoriteRestaurant || [];
...
const addFavorite = async (
  ...
  try {
    ...
    }>(apiUrl + "/users/token", options);

    const currentFavorites = response.favoriteRestaurant || [];
    console.log("Current favorite restaurants:", currentFavorites);

    if (currentFavorites.includes(restaurantId)) {
      console.warn("Restaurant is already a favorite.");
      return;
    }
    ...
    console.log('Users favorites fetched after update:', response.favoriteRestaurant);
