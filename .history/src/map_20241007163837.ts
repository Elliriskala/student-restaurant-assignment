...
import { addFavorite, removeFavorite, showFavorites } from "./favorites";
import { fetchFavorites } from "./main";
...
const initializePopup = (restaurant: Restaurant, marker: mapboxgl.Marker) => {
  ...
  const removeFavoriteButton = document.createElement("button") as HTMLButtonElement;

  const favorites = JSON.parse(localStorage.getItem("favorites") || "[]");

  const isFavorite = favorites.some((favorite: { name: string; }) => favorite.name === restaurant.name);

  favoriteButton.innerText = "Add to favorites";
  removeFavoriteButton.innerText = "Remove favorite";
  ...
  favoriteButton.addEventListener("click", async (event) => {
    ...
    const updateFavorites = await fetchFavorites(localStorage.getItem("token") || "");
    showFavorites(updateFavorites);
  });

  removeFavoriteButton.addEventListener("click", async (event) => {
    event.preventDefault();
    await removeFavorite(restaurant.name);
    removeFavoriteButton.style.display = "none";
  ...
  removeFavoriteButton.classList.add("favorite-button");
  ...
  favoriteDiv.appendChild(removeFavoriteButton);
