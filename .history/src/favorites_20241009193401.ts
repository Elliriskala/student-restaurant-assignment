...
const fetchFavoriteRestaurantId = async (
  ...
  try {
    const userData = await fetchData<{ favoriteRestaurant: string | null}>(apiUrl + "/users/token", {
      method: "GET",
      headers: {
    ...
    return userData.favoriteRestaurant || null;
  } catch (error) {
...
const fetchRestaurantId = async (
  ...
  try {
    const restaurantData = await fetchData<{ name: string, address: string}>(
      `https://media1.edu.metropolia.fi/restaurant/api/v1/restaurants/${restaurantId}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
