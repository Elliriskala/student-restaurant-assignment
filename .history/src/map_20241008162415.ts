...
import { todayModal, weekModal, errorModal, favoritesSuccessModal } from "./components";
...
const restaurantSelection = async (restaurantId: string, restaurantName: string): Promise<void> => {
  ...
  try {
    ...
    const modal = document.querySelector("#favorites-success-modal") as HTMLDialogElement;
    if (modal) {
      modal.innerHTML = favoritesSuccessModal(restaurantName);
      modal.showModal();
    } else {
      console.error("Favorites success modal not found");
    }
  } catch (error) {
...
const initializePopup = (restaurant: Restaurant, marker: mapboxgl.Marker) => {
  ...
  favoriteButton.addEventListener("click", () => { restaurantSelection(restaurant._id, restaurant.name) });
